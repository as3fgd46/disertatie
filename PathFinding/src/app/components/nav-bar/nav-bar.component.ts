...
import { ShowAlgorithmsComponent } from '../show-algorithms/show-algorithms.component';
...
export class NavBarComponent implements OnInit {
    ...
    for (let i = 0; i < this.shapes.length; i++) {
      for (let j = 0; j < this.shapes[i].length; j++) {
        ...
        if (i == 4 && j == 4) {
          ...
          i == this.canvas.width / this.shapedimension - 5 &&
          j == this.canvas.height / this.shapedimension - 5
        ) {
        ...
        this.shapes[i][j] = {
          ...
          cameFrom,
          visited,
        }; //x and y are grid coordinates, and i j is the index in array the square object is in, and type is the type of the node, FGH is a* related info
      }
    }
    //a_star_search();
  }

  async draw_erase_walls(e: { which }, cx, cy) {
    //mouse pressed
    if (e.which == 1 && !this.disableButtons) {
      ...
      for (let i = 0; i < this.shapes.length; i++) {
        for (let j = 0; j < this.shapes[i].length; j++) {
          if (
            cx < this.shapes[i][j].x + 12 &&
            cx > this.shapes[i][j].x &&
            cy < this.shapes[i][j].y + 12 &&
            cy > this.shapes[i][j].y
          ) {
  ...
  async changeEnd(cx, cy) {
    for (let i = 0; i < this.shapes.length; i++) {
      for (let j = 0; j < this.shapes[i].length; j++) {
        if (
          cx < this.shapes[i][j].x + this.shapedimension &&
          cx > this.shapes[i][j].x &&
          cy < this.shapes[i][j].y + this.shapedimension &&
          cy > this.shapes[i][j].y
        ) {
            ...
            for (let k = 0; k < this.shapes.length; k++) {
              for (let l = 0; l < this.shapes[k].length; l++) {
                if (this.shapes[k][l].type == 'End') {
                  ...
                  this.ctxGrid.fillStyle = '#3d3d3d';
                  this.ctxGrid.clearRect(
                    this.shapes[k][l].x - 0.5,
                    this.shapes[k][l].y - 0.5,
                    this.shapedimension + 0.6,
                    this.shapedimension + 0.5
                  );
                  this.ctxGrid.strokeRect(
                    this.shapes[k][l].x,
                    this.shapes[k][l].y,
      ...
      for (let j = 0; j < this.shapes[0].length; j++) {
        if (
          ...
          if (rand < 0.35) {
            ...
            this.ctxGrid.fillRect(
              this.shapes[i][j].x + 0.5,
              this.shapes[i][j].y + 0.5,
              this.shapedimension - 1,
              this.shapedimension - 1
            );
          }
  ...
  clearSearchNotWalls() {
    ...
    for (let i = 0; i < wallPositions.length; i++) {
      this.ctxGrid.fillStyle = '#3d3d3d';
      this.ctxGrid.fillRect(
        wallPositions[i].x + 0.5,
        wallPositions[i].y + 0.5,
        this.shapedimension - 1,
        this.shapedimension - 1
      );
    }
  ...
  returnNeighbors(node: { i: number; j: number }) {
  ...
  openCreatePostDialog() {
    const dialogRef = this.dialog.open(HowToComponent);
  ...
  openVisualizeAlgorithms() {
    const dialogRef2 = this.dialog.open(ShowAlgorithmsComponent);
